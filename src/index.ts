// @ts-ignore
import * as pant from 'nearest-pantone';
...
function parseRGB(hexString: string) {
    // Convert the hex string to a number
    const hex = parseInt(hexString, 16);

    // Extract the R, G, and B values from the hex number
    const r = (hex >> 16) & 255;
    const g = (hex >> 8) & 255;
    const b = hex & 255;

    // Return the R, G, and B values as a JSON object
    return { R: r, G: g, B: b };
}

function RGB2HEX(r: number, g: number, b: number) {
    // Convert the R, G, and B values to hex strings and pad with zeros if needed
...
app.get('/pantone', (req, res) => {
  // Get the hex value from the query parameter
  const inputColor = req.query.hex as string;

  const inputColorRGB = inputColor;

  /*
  // Call the pant.getClosestColor function to get the nearest Pantone color
  const pantoneColor = pant.getClosestColor(inputColor);

  // Send the Pantone color as a JSON response
  res.json(pantoneColor);
  */
 
  // Find the closest color to the input color
  const closestColor = closest(parseRGB(inputColorRGB), colors.map(color => parseRGB(color.hex)));
  console.log( closestColor);
  let hex = "" + RGB2HEX(closestColor.R, closestColor.G, closestColor.B);
  const foundColor = colors.find(color => color.hex === hex);

  // Send the closest color as a JSON response
  if(foundColor) {
    res.json({
...
app.listen(port, () => {
  console.log(`Pantone lookup app listening at http://localhost:${port}`);
});
